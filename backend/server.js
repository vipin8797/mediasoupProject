...
io.on("connection", (socket) => {
  ...
  //ProducerTransport connect event
  socket.on("connect-transport", async ({ dtlsParameters, type }, ack) => {
    try {
      if (type === "producer") {
        ...
        // TODO: connect downstream transport
      }
      console.log("connect.....");
    } catch (err) {
      console.log(err);
  ...
  //ProducerTransport produce event
  socket.on("start-producing", async ({ kind, rtpParameters }, ack) => {
    try {
        console.log("produce event start");
      const newProducer = await client.upstreamTransport.produce({
        kind,
        rtpParameters,
      });

      //TODO: if audio track is avalaible then it is possible that it may be Dominant speaker in Room objc
      //TODO: There are other memebers in Room ,notify them
